...
import com.yyl.locationservice.database.LocContentProvider;
import com.yyl.locationservice.database.LocTable;
...
//    Toast.makeText(this, "service done", Toast.LENGTH_SHORT).show(); 
  ...
  private String trackLocation(Location location) {
	  ...
	  if (location != null)
	  {
  		...
  		Cursor lastRow = getContentResolver().query(LocContentProvider.CONTENT_URI, new String[] { LocTable.COLUMN_ID, LocTable.COLUMN_TIME, 
     			LocTable.COLUMN_LONGITUDE, LocTable.COLUMN_LATITUDE }, null, null, null);
  		lastRow.moveToFirst();
  		String lastLongitude = lastRow.getString(2);
  		String lastLatitude = lastRow.getString(3);
  		getContentResolver().insert(LocContentProvider.CONTENT_URI, values);
  		result = "Location: from " + lastLongitude+", "+lastLatitude + " to: "+Double.toString(longitude)+", "+Double.toString(latitude);
	  }
		return result;
  }
